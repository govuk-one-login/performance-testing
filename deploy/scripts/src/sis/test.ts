...
const env = {
  ...
  envUserIdentityURL: getEnv('USERIDENTITY_SIS_URL'),
  envUserIdentityApiKey: getEnv('USERIDENTITY_SIS_API_KEY'),
  envUserIdentitykeyID: getEnv('USERIDENTITY_SIS_KID') //did:web:reuse-identity-shared-core-stub.reuse-identity.build.account.gov.uk:perf-test#perf-test-key
}
const keys = {
...
export async function useridentity(): Promise<void> {
  ...
  // B03_SIS_01_PostVC-StubCall
  timeGroup(groups[0], () => http.post(env.EVCSStubURL + `/vcs/${userID}`, body, options), {
  ...
  const createJwt = async (key: JsonWebKey, payload: object): Promise<string> => {
    const escdaParam: EcKeyImportParams = { name: 'ECDSA', namedCurve: 'P-256' }
    const importedKey = await crypto.subtle.importKey('jwk', key, escdaParam, true, ['sign'])
    return signJwt('ES256', importedKey, payload, env.envUserIdentitykeyID) //"kid": "did:web:reuse-identity-shared-core-stub.reuse-identity.build.account.gov.uk:perf-test#perf-test-key"
  }
  const identityJWT = await createJwt(keys.useridentity, payloads.useridentityPayload)
  //B03_SIS_02_PostSISrecord-StubCall
  const identityReqBody = JSON.stringify({
    userId: subjectID,
    si: {
  ...
  timeGroup(groups[1], () => http.post(env.EVCSStubURL + '/v1/identity', identityReqBody, params), {
    isStatusCode202
  })
  //B03_SIS_03_GetToken-StubCall
  ...
  const tokenParams = {
    headers: {
      'x-api-key': env.MockTokenApiKey
    }
  }
  const res: Response = timeGroup(
    groups[2],
    () => http.post(env.MockTokenURL + '/generate', tokenPayload, tokenParams),
  ...
  const useridentityReqBody = JSON.stringify({
    vtr: 'P2',
    govukSigninJourneyId: userID
  })

  timeGroup(groups[3], () => http.post(env.envUserIdentityURL + '/user-identity', useridentityReqBody, parameters), {
    isStatusCode204
  })

  iterationsCompleted.add(1)
}
/// Additional local functions used ///
