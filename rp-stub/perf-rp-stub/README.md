...
```bash
perf-rp-stub$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
perf-rp-stub$ sam logs -n HelloWorldFunction --stack-name perf-rp-stub --tail
```
