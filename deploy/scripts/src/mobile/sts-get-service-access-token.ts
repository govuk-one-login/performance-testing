...
export async function getServiceAccessToken(): Promise<void> {
  ...
  const orchestrationAuthorizeUrl = getAuthorize(groupMap.getServiceAccessToken[0], codeChallenge)
  ...
  const stsAuthorizationCode = getRedirect(groupMap.getServiceAccessToken[3], state, orchestrationAuthorizationCode)
