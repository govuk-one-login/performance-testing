...
export function passport (): void {
  ...
  res = group(groups[0], () => timeRequest(() => // B01_Passport_01_LaunchOrchestratorStub
  ...
  res = group(groups[1], () => timeRequest(() => { // B01_Passport_02_GoToFullJourneyRoute
    const response = http.get(env.orchStubEndPoint + `/authorize?journeyType=full&userIdText=${userId}&signInJourneyIdText=${signInJourneyId}&vtrText=Cl.Cm.P2&targetEnvironment=${environment}&reproveIdentity=NOT_PRESENT&emailAddress=${testEmail}&votText=&jsonPayload=&evidenceJsonPayload=&error=recoverable`,
      {
        headers: { Authorization: `Basic ${encodedCredentials}` }
      })
  ...
  group(groups[2], () => { // B01_Passport_03_ClickContinueStartPage
    timeRequest(() => {
      res = group(groups[3].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          fields: { journey: 'next' },
          params: { redirects: 0 }
        }),
      { isStatusCode302 }))
      res = group(groups[4].split('::')[1], () => timeRequest(() => // 02_DCMAWStub
  ...
  group(groups[5], () => { // B01_Passport_04_DCMAWContinue
    timeRequest(() => {
      res = group(groups[6].split('::')[1], () => timeRequest(() => // 01_DCMAWStub
        res.submitForm({
          fields: {
      ...
      { isStatusCode302 }))
      res = group(groups[7].split('::')[1], () => timeRequest(() => // 02_CoreCall
        http.get(env.ipvCoreURL + res.headers.Location),
  ...
  group(groups[8], () => { // B01_Passport_05_ContinueOnPYIStartPage
    timeRequest(() => {
      res = group(groups[9].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          fields: { journey: 'next/passport' },
          params: { redirects: 0 }
        }),
      { isStatusCode302 }))
      res = group(groups[10].split('::')[1], () => timeRequest(() => // 02_PassStub
        http.get(res.headers.Location),
  ...
  group(groups[11], () => { // B01_Passport_06_PassportDataContinue
    timeRequest(() => {
      res = group(groups[12].split('::')[1], () => timeRequest(() => // 01_PassStub
        res.submitForm({
          fields: {
      ...
      { isStatusCode302 }))
      res = group(groups[13].split('::')[1], () => timeRequest(() => // 02_CoreCall
        http.get(res.headers.Location, { redirects: 0 }),
      { isStatusCode302 }))
      res = group(groups[14].split('::')[1], () => timeRequest(() => // 03_AddStub
        http.get(res.headers.Location),
  ...
  group(groups[15], () => { // B01_Passport_07_AddrDataContinue
    timeRequest(() => {
      res = group(groups[16].split('::')[1], () => timeRequest(() => // 01_AddStub
        res.submitForm({
          fields: { jsonPayload: addressPayloadP },
          params: { redirects: 0 }
        }),
      { isStatusCode302 }))
      res = group(groups[17].split('::')[1], () => timeRequest(() => // 02_CoreCall
        http.get(res.headers.Location, { redirects: 0 }),
      { isStatusCode302 }))
      res = group(groups[18].split('::')[1], () => timeRequest(() => // 03_FraudStub
        http.get(res.headers.Location),
  ...
  group(groups[19], () => { // B01_Passport_08_FraudDataContinue
    timeRequest(() => {
      res = group(groups[20].split('::')[1], () => timeRequest(() => // 01_FraudStub
        res.submitForm({
          fields: {
      ...
      { isStatusCode302 }))
      res = group(groups[21].split('::')[1], () => timeRequest(() => // 02_CoreCall
        http.get(res.headers.Location),
  ...
  group(groups[22], () => { // B01_Passport_09_PreKBVTransition
    timeRequest(() => {
      res = group(groups[23].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          params: { redirects: 0 }
        }),
      { isStatusCode302 }))
      res = group(groups[24].split('::')[1], () => timeRequest(() => // 02_KBVStub
        http.get(res.headers.Location),
  ...
  group(groups[25], () => { // B01_Passport_10_KBVDataContinue
    timeRequest(() => {
      res = group(groups[26].split('::')[1], () => timeRequest(() => // 01_KBVStub
        res.submitForm({
          fields: {
      ...
      { isStatusCode302 }))
      res = group(groups[27].split('::')[1], () => timeRequest(() => // 02_CoreCall
        http.get(res.headers.Location),
  ...
  group(groups[28], () => { // B01_Passport_11_ContinuePYISuccessPage
    timeRequest(() => {
      res = group(groups[29].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          params: { redirects: 0 }
        }),
      { isStatusCode302 }))
      res = group(groups[30].split('::')[1], () => timeRequest(() => // 02_OrchStub
        http.get(res.headers.Location, {
          headers: { Authorization: `Basic ${encodedCredentials}` }
        }),
...
export function drivingLicence (): void {
  ...
  res = group(groups[0], () => timeRequest(() => // B02_DrivingLicence_01_LaunchOrchestratorStub
    http.get(env.orchStubEndPoint, {
      headers: { Authorization: `Basic ${encodedCredentials}` }
    }),
  { isStatusCode200, ...pageContentCheck('Enter userId manually') }))
  ...
  res = group(groups[1], () => timeRequest(() => { // B02_DrivingLicence_02_SelectUserIDContinue
    const response = http.get(env.orchStubEndPoint + `/authorize?journeyType=full&userIdText=${userId}&signInJourneyIdText=${signInJourneyId}&vtrText=Cl.Cm.P2&targetEnvironment=${environment}&reproveIdentity=NOT_PRESENT&emailAddress=${testEmail}&votText=&jsonPayload=&evidenceJsonPayload=&error=recoverable`,
      {
        headers: { Authorization: `Basic ${encodedCredentials}` }
      })
  ...
  group(groups[2], () => { // B02_DrivingLicence_03_ContinueStartPage
    timeRequest(() => {
      res = group(groups[3].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          fields: { journey: 'next' },
  ...
  group(groups[5], () => { // B02_DrivingLicence_04_DCMAWContinue
    timeRequest(() => {
      res = group(groups[6].split('::')[1], () => timeRequest(() => // 01_DCMAWStub
        res.submitForm({
          fields: {
  ...
  group(groups[8], () => { // B02_DrivingLicence_05_ContinueOnPYIStartPage
    timeRequest(() => {
      res = group(groups[9].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
      ...
      { isStatusCode302 }))
      res = group(groups[10].split('::')[1], () => timeRequest(() => // 02_DLStub
        http.get(res.headers.Location),
  ...
  group(groups[11], () => { // B02_DrivingLicence_06_DrivingLicenceDataContinue
    timeRequest(() => {
      res = group(groups[12].split('::')[1], () => timeRequest(() => // 01_DLStub
        res.submitForm({
          fields: {
  ...
  group(groups[15], () => { // B02_DrivingLicence_07_AddrDataContinue
    timeRequest(() => {
      res = group(groups[16].split('::')[1], () => timeRequest(() => // 01_AddStub
        res.submitForm({
  ...
  group(groups[19], () => { // B02_DrivingLicence_08_FraudDataContinue
    timeRequest(() => {
      res = group(groups[20].split('::')[1], () => timeRequest(() => // 01_FraudStub
        res.submitForm({
          fields: {
  ...
  group(groups[22], () => { // B02_DrivingLicence_09_PreKBVTransition
    timeRequest(() => {
      res = group(groups[23].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          params: { redirects: 0 }
  ...
  group(groups[25], () => { // B02_DrivingLicence_10_KBVDataContinue
    timeRequest(() => {
      res = group(groups[26].split('::')[1], () => timeRequest(() => // 01_KBVStub
        res.submitForm({
          fields: {
  ...
  group(groups[28], () => { // B02_DrivingLicence_11_ContinueDrivingLicenceSuccessPage
    timeRequest(() => {
      res = group(groups[23].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          params: { redirects: 0 }
      ...
      { isStatusCode302 }))
      res = group(groups[24].split('::')[1], () => timeRequest(() => // 02_OrchStub
        http.get(res.headers.Location, {
          headers: { Authorization: `Basic ${encodedCredentials}` }
        }),
      { isStatusCode200, ...pageContentCheck('User information') }))
...
export function idReuse (): void {
  ...
  group(groups[0], () => { // B03_IDReuse_01_LoginToCore
    timeRequest(() => {
      res = group(groups[1].split('::')[1], () => timeRequest(() => // 01_OrchStub
        http.get(env.orchStubEndPoint + `/authorize?journeyType=full&userIdText=${idReuseUserID.userID}&signInJourneyIdText=${signInJourneyId}&vtrText=P2&targetEnvironment=${environment}&reproveIdentity=NOT_PRESENT&emailAddress=${idReuseUserID.emailID}&votText=&jsonPayload=&evidenceJsonPayload=&error=recoverable`,
          {
            headers: { Authorization: `Basic ${encodedCredentials}` },
            redirects: 0
          }),
      { isStatusCode302 }))
      res = group(groups[2].split('::')[1], () => timeRequest(() => // 02_CoreCall
        http.get(res.headers.Location),
  ...
  group(groups[3], () => { // B03_IDReuse_02_ClickContinue
    timeRequest(() => {
      res = group(groups[4].split('::')[1], () => timeRequest(() => // 01_CoreCall
        res.submitForm({
          fields: { submitButton: '' },
          params: { redirects: 0 },
      ...
      { isStatusCode302 }))
      res = group(groups[5].split('::')[1], () => timeRequest(() => // 02_OrchStub
