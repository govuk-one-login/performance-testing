...
export function BAV (): void {
  ...
  res = group('B01_BAV_01_IPVStubCall POST', () =>
    timeRequest(() => http.post(env.BAV.ipvStub + '/start',
      {
        'shared_claims.name.nameParts[0].value': 'Yasmine',
        'shared_claims.name.nameParts[0].type': 'GivenName',
        'shared_claims.name.nameParts[1].value': 'Young',
        'shared_claims.name.nameParts[1].type': 'FamilyName',
        'shared_claims.birthDate.value': '1960-02-02'
      },
      {
