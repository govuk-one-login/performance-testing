...
const profiles: ProfileList = {
  ...
  perf006Iteration5SpikeTest: {
    ...createI3SpikeOLHScenario('changeEmail', 10, 24, 1),
    ...createI3SpikeOLHScenario('changePassword', 10, 21, 1),
    ...createI3SpikeOLHScenario('changePhone', 10, 24, 1),
    ...createI3SpikeOLHScenario('deleteAccount', 10, 18, 1),
    ...createI3SpikeSignInScenario('landingPage', 16, 6, 8)
  },
  ...
  perf006Iteration10PeakTest: {
    ...createOLHPeakTestScenario('changeEmail', 17, 34, 1),
...
const groupMap = {
  ...
  removePasskey: [
    ...
    'B08_RemovePasskey_01_LaunchAccountsHome::01_OLHCall',
    'B08_RemovePasskey_01_LaunchAccountsHome::02_OIDCStubCall',
    'B08_RemovePasskey_02_SelectStubScenario',
    'B08_RemovePasskey_02_SelectStubScenario::01_OIDCStubCall',
    'B08_RemovePasskey_02_SelectStubScenario::02_OLHCall',
...
export function changePassword(): void {
  ...
  res = timeGroup(
    groups[10],
    () => {
      console.log('=== DEBUG: Looking for change-password form ===')
      console.log('Current URL:', res.url)
      console.log('Response status:', res.status)
      console.log('Response body length:', res.body ? res.body.length : 'no body')
      console.log('Response content type:', res.headers['Content-Type'] || 'no content type')

      // Log first 1000 chars to see page content
      if (res.body) {
        console.log('First 1000 chars:', res.body.substring(0, 1000))
      }

      const allForms = res.html('form')
      console.log(`Found ${allForms.length} forms`)

      // Check if page has any HTML content
      const htmlElements = res.html('*')
      console.log(`Total HTML elements found: ${htmlElements.length}`)

      // Check for error indicators
      const bodyText = res.html('body').text()
      if (bodyText && (bodyText.includes('error') || bodyText.includes('problem') || bodyText.includes('sorry'))) {
        console.log('ERROR PAGE DETECTED:', bodyText.substring(0, 200))
      }
...
export function removePasskey(): void {
  const groups = groupMap.removePasskey
  let res: Response
  iterationsStarted.add(1)
  ...
  // B08_RemovePasskey_07_ConfirmRemovePasskey
  const csrfToken2 = res.html('input[name="_csrf"]').attr('value') ?? ''
